...
function Home(){
  ...
  const { sendMessage:sendBroadcast, lastMessage:lastBroadcast } = useWebSocket(SOCKET_URL, {
  ...
  useEffect(() => {
    if (lastBroadcast !== null) {
      try {
        ...
        setBroadcasts((prev) => [...prev, { text: parsedBroadcast.text, sent_by: parsedBroadcast.sent_by,username:parsedBroadcast.username, image_url:parsedBroadcast.image_url }]);
      } catch (error) {
  ...
  const handleSendBroadcast = async () => {
    ...
    if (image){
      ...
      try {
        ...
        if(response.status===201){
          sendBroadcast(JSON.stringify({ sent_by: userId, image_url: response.data.image_url, username:username}));
        }
      } catch (error: any) {
        console.error(error);
    ...
    if(broadcast.trim() !== ""){
      ...
      sendBroadcast(JSON.stringify({ sent_by: userId, text: broadcast, username:username }));
      setBroadcast("");
    }
  };
  ...
  const [posts,setPosts] = useState<{user_id:number,post:string,username:string,image_url:string}[]>([])
            ...
            <div className='h-[85vh]'>
              <div className='border bg-white w-full h-[95%] overflow-auto ' ref={chatContainerRef}>
                {posts.map((post,index)=>(
                  <div key={index} className={`p-1 border-b border-gray-100 my-5 ${post.user_id === userId ? "text-right" : "text-left"}`}>
                    <span
                      className={`border p-2 rounded-xl flex ${
                        post.user_id === userId ? "border-blue-700 bg-sky-300" : "border-green-700 text-right bg-green-400"
                      }`}
                    >
                      <div className='w-full mx-5'>
                        <div className='flex justify-between'>
                          <p className='font-bold cursor-pointer border rounded-lg px-1 bg-gray-100' onClick={()=>ViewProfile(post.user_id)}>{post.username}</p>
                          {username !== post.username ? (users.some(user => user.username === post.username) ?
                          (<button className='border px-2 cursor-pointer rounded bg-black text-white font-semibold' onClick={()=>Unfollow(post.user_id)}><span className='flex'>Followed<CircleCheck size={18} className='ml-2 mt-1'/></span></button>
                          ):(
                            <button className='border hover:bg-lime-300 px-2 cursor-pointer rounded bg-white' onClick={()=>Follow(post.user_id)}>Follow</button>
                          )):("")}
                        </div>
                ...
                {broadcasts.map((brdcst, index) => (
                  <div
                    key={index}
                    className={`p-1 border-b border-gray-100 my-5 ${brdcst.sent_by === userId ? "text-right" : "text-left"}`}
                  >
                    <span
                      className={`border p-2 rounded-xl flex ${
                        brdcst.sent_by === userId ? "border-blue-700 bg-sky-300" : "border-green-700 text-right bg-green-400"
                      }`}
                    >
                      <div className='w-full mx-5'>
                        <div className='flex justify-between'>
                          <p className='font-bold cursor-pointer border rounded-lg px-1 bg-gray-100' onClick={()=>ViewProfile(brdcst.sent_by)}>{brdcst.username}</p>
                          {username !== brdcst.username ? (users.some(user => user.username === brdcst.username) ?
                          (<button className='border px-2 cursor-pointer rounded bg-black text-white font-semibold' onClick={()=>Unfollow(brdcst.sent_by)}><span className='flex'>Followed<CircleCheck size={18} className='ml-2 mt-1'/></span></button>
                          ):(
                            <button className='border hover:bg-lime-300 px-2 cursor-pointer rounded bg-white' onClick={()=>Follow(brdcst.sent_by)}>Follow</button>
                          )):("")}
                        </div>
                        <div className='text-left mt-2 break-words'>
