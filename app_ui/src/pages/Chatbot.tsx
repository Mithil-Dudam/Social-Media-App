...
function Chatbot(){
    ...
    const {userChoice, userId, username, setUserChoice, setProfile, setError} = useAppContext()
    ...
    const ToAIChatbot = () => {
      ...
      const [chatbotChats,setChatbotChats] = useState<{text:string,sent_by:string}[]>([])
            ...
            <div className='h-[85vh]'>
              <div className='border bg-white w-full h-[95%] overflow-auto ' ref={chatContainerRef}>
                {chatbotChats.map((text,index)=>(
                  <div key={index} className={`p-1 border-b border-gray-100 my-5 ${text.sent_by === "user" ? "text-right" : "text-left"}`}>
                    {text.text&&<div
                    className={`inline-block p-3 rounded-xl whitespace-pre-wrap break-words ${
                        text.sent_by === "user" ? "bg-sky-300 border border-blue-700 self-end text-right" : "bg-green-400 border border-green-700 self-start text-left"
                    }`}
                    >
                    {text.text}
                    </div>
