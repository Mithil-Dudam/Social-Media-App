...
function Chats() {
  ...
  useEffect(() => {
    if (lastMessage !== null) {
      try {
        ...
        setMessages((prev) => [...prev, { text: parsedMessage.text, sent_by: parsedMessage.sent_by, image_url:parsedMessage.image_url, chat_id:parsedMessage.chat_id}]);
      } catch (error) {
  ...
  const handleSendChat = async () => {
    ...
    if (image){
      ...
      try {
        const response = await api.post(`/text?chat_id=${chatId}&sent_by=${userId}`,  formData, {
          headers: { "Content-Type": "multipart/form-data" }});
        if(response.status===201){
          sendMessage(JSON.stringify({ sent_by: userId, image_url: response.data.image_url, chat_id:chatId}));
        }
      } catch (error: any) {
        console.error(error);
    ...
    sendMessage(JSON.stringify({ sent_by: userId, text: message, chat_id:chatId}));
    setMessage("");
    };
  };

  const [allTexts,setAllTexts] = useState<{text:string,sent_by:number, image_url:string}[]>([])
    ...
    return(
        <div className='min-w-screen min-h-screen bg-fuchsia-300 flex'>
          ...
          <div className='w-full border pb-10 pl-10 pr-10 pt-5'>
            <MoveLeft className='bg-white rounded-full mb-1 cursor-pointer' onClick={ToUserList}/>
            <div className='h-[85vh]'>
              <div className='border bg-white w-full h-[95%] overflow-auto ' ref={chatContainerRef}>
                {allTexts.map((text,index)=>(
                  <div key={index} className={`p-1 border-b border-gray-100 my-5 ${text.sent_by === userId ? "text-right" : "text-left"}`}>
                    {text.text&&<span
                      className={`border p-2 rounded-xl ${
                        text.sent_by === userId ? "border-blue-700 bg-sky-300" : "border-green-700 text-right bg-green-400"
                      }`}
                    >
                      <span className='mt-2 break-words'>{text.text}</span>
                    </span>}
                ...
                {messages.map((msg, index) => (
                  <div
                    key={index}
                    className={`p-1 border-b border-gray-100 my-5 ${msg.sent_by === userId ? "text-right" : "text-left"}`}
                  >
                    {msg.text&&<span
                      className={`border p-2 rounded-xl ${
                        msg.sent_by === userId ? "border-blue-700 bg-sky-300" : "border-green-700 text-right bg-green-400"
                      }`}
                    >
                      <span className='mt-2 break-words'>{msg.text}</span>
                    </span>}
